...
func TestSyncHappyPath(t *testing.T) {
				...
				ExpectedActions: []testpkg.Action{
					testpkg.NewCustomMatch(coretesting.NewCreateAction(corev1.SchemeGroupVersion.WithResource("secrets"), gen.DefaultTestNamespace, nil),
						func(exp, _ coretesting.Action) bool {
							actSecret := actual.(coretesting.CreateAction).GetObject().(*corev1.Secret)
							if len(actSecret.Data[corev1.TLSCertKey]) == 0 {
								return false
							}
							if len(actSecret.Data[corev1.TLSPrivateKeyKey]) == 0 {
								return false
							}
							return true
						}),
	...
	for n, test := range tests {
		t.Run(n, func(t *testing.T) {
			...
			requeue, err := test.Controller.Sync(test.Ctx, crtCopy)
			if err != nil && !test.Err {
			...
			test.Finish(t, crtCopy, requeue, err)
		})
	}
}
